 ...
 * 
 *
 * 注意: All these enumerations should be casted to (unsigned char) before writing them to RakNet::BitStream
 */
enum DefaultMessageIDTypes
{
	...
	/*
	 * 内部使用, do not process in your game
	 ...
	 */
	ID_FCM2_NEW_HOST,
	/* 内部使用 For FullyConnectedMesh2 plugin */
	 ...
	 * 注意: Only the designated client gets this message
	 * 注意: You won't get this message if you are already connected to all target systems
	 * 注意: If you fail to connect to a system, this does not automatically mean you will get ID_FCM2_VERIFIED_JOIN_FAILED as that system may have been shutting down from the host too
	 * 另见 FullyConnectedMesh2::StartVerifiedJoin()
	 */
	ID_FCM2_VERIFIED_JOIN_START,
	/* 内部使用 The client has completed processing for all systems designated in ID_FCM2_VERIFIED_JOIN_START */
	ID_FCM2_VERIFIED_JOIN_CAPABLE,
	/*
	 * Client failed to connect to a 必须 systems notified via FullyConnectedMesh2::StartVerifiedJoin()
	 ...
	 * 注意: Server normally sends us this message, however if connection to the server was lost, message will be returned locally
	 * 注意: Only the designated client gets this message
	 */
	 ...
	 * 注意: All systems in the mesh get this message
	 * 另见 RespondOnVerifiedJoinCapable()
	 */
	...
	/* 内部使用 Used by the team balancer plugin */
	 ...
	 *  获取 ID_TEAM_BALANCER_TEAM_ASSIGNED later
