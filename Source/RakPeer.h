...
public:
	 ...
	 * 注意: Multiple calls while already active are ignored.  To call this function again with different settings, you must first call Shutdown().
	 * 注意: Call SetMaximumIncomingConnections if you want to accept incoming connections.
	 * 参数[输入] maxConnections Maximum number of connections between this instance of RakPeer and another instance of RakPeer. 必须 so that the network can preallocate and for thread safety. A pure client would set this to 1.  A pure server would set it to the number of allowed clients.A hybrid would set it to the sum of both types of connections.
	 ...
	 * 注意: If secure connections are on, do not use secure connections for a specific IP address.
	 ...
	 * 返回值: True if the IP address is found in security exception list, else returns false.
	 */
	bool IsInSecurityExceptionList(const char *ip);

	/*
	 * 设置 maximum number of incoming connections allowed
	 * If the number of incoming connections is less than the number of players currently connected,
	 ...
	 * 返回值: Maximum number of incoming connections, which is always <= maxConnections
	 */
	unsigned int GetMaximumIncomingConnections( void ) const;

	/*
	 * 返回当前存在的已打开连接数。
	 * 返回值: Number of open connections.
	 */
	unsigned short NumberOfConnections(void) const;

	/*
	 * 设置 password for the incoming connections
	 * The password must match in the call to Connect (defaults to 无).
	 ...
	 * 参数[输入/输出] passwordDataLength Maximum size of the passwordData array.  Modified to hold the number of bytes actually written.
	 */
	 ...
	 * 参数[输入] remotePort Port to connect to on the remote machine.
	 ...
	 * 参数[输入] connectionSocketIndex Index into the array of socket descriptors passed to socketDescriptors in RakPeer::Startup() to determine the one to send on.
	 * 参数[输入] sendConnectionAttemptCount Number of datagrams to send to the other system to try to connect.
	 ...
	 * 参数[输入] timeoutTime Time to elapse before dropping the connection if a reliable message could not be sent. 0 to use the default value from SetTimeoutTime(UNASSIGNED_SYSTEM_ADDRESS);
	 ...
	 * 参数[输入] sendConnectionAttemptCount Number of datagrams to send to the other system to try to connect.
	 * 参数[输入] timeBetweenSendConnectionAttemptsMS Time to elapse before a datagram is sent to the other system to try to connect. After sendConnectionAttemptCount number of attempts, ID_CONNECTION_ATTEMPT_FAILED is returned.. Under low bandwidth conditions with multiple simultaneous outgoing connections, this value should be raised to 1000 or higher, or else the MTU detection can overrun the available bandwidth.
	 * 参数[输入] timeoutTime Time to elapse before dropping the connection if a reliable message could not be sent. 0 to use the default from SetTimeoutTime(UNASSIGNED_SYSTEM_ADDRESS);
	 * 返回值: ConnectionAttemptResult::CONNECTION_ATTEMPT_STARTED on successful initiation. Otherwise, an appropriate enumeration indicating failure.
	 * 注意: ConnectionAttemptResult::CONNECTION_ATTEMPT_STARTED does not mean you are already connected!
	 */
	 ...
	 * 参数[输入] networkServiceId Network ID structure for the online service
	 * 参数[输入] passwordData A data block that must match the data block on the server passed to SetIncomingPassword().  This can be a string or can be a stream of data.  Use 0 for no password.
	 * 参数[输入] passwordDataLength The length in bytes of passwordData.
	 */
	 ...
	 * 参数[输入] blockDuration Wait time(milli seconds) for all remaining messages to go out, including ID_DISCONNECTION_NOTIFICATION.  If 0, it doesn't wait at all.
	 * 参数[输入] orderingChannel Channel on which ID_DISCONNECTION_NOTIFICATION will be sent, if blockDuration > 0.
	 * 参数[输入] disconnectionNotificationPriority Priority of sending ID_DISCONNECTION_NOTIFICATION.
	...
	/*
	 * 如果the network thread is running则返回 true
	 ...
	 * 参数[输出] remoteSystems An array of SystemAddress structures, to be filled with the SystemAddresss of the systems we are connected to. Pass 0 to remoteSystems to get the number of systems we are connected to.
	...
	/*
	 * 向你已连接的指定系统发送一个数据块。
	 * 注意: This function only works while connected.
	 * 注意: The first byte should be a message identifier starting at ID_USER_PACKET_ENUM.
	 * 参数[输入] data Block of data to send.
	 * 参数[输入] length Size in bytes of the data to send.
	 * 参数[输入] priority Priority level to send on.  See PacketPriority.h
	 * 参数[输入] reliability How reliably to send this data.  See PacketPriority.h
	 * 参数[输入] orderingChannel When using ordered or sequenced messages, the channel to order these on. Messages are only ordered relative to other messages on the same stream.
	 ...
	 * The message will be processed through the plugins and returned to the game as usual.
	 ...
	 * 向你已连接的指定系统发送一个数据块。
	 *
	 * 与 the above version, but takes a BitStream as input 相同
	 * 参数[输入] bitStream Bitstream to send
	 * 参数[输入] priority Priority level to send on.  See PacketPriority.h
	 * 参数[输入] reliability How reliably to send this data.  See PacketPriority.h
	 * 参数[输入] orderingChannel Channel to order the messages on, when using ordered or sequenced messages. Messages are only ordered relative to other messages on the same stream.
	 * 参数[输入] systemIdentifier System Address or RakNetGUID to send this packet to, or in the case of broadcasting, the address not to send it to.  Use UNASSIGNED_SYSTEM_ADDRESS to specify 无.
	 * 参数[输入] broadcast True to send this packet to all connected systems. If true, then systemAddress specifies who not to send the packet to.
	 * 参数[输入] forceReceipt If 0, will automatically determine the receipt number to return. If non-zero, will return what you give it.
	 * 返回值: 0 on bad input. Otherwise a number that identifies this message. If reliability is a type that returns a receipt, on a later call to Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive containing this number
	 ...
	 * 另见 RakNetTypes.h contains struct Packet.
	 */
	Packet* Receive( void );

	/*
	 * Call this to deallocate a message returned by Receive() when you are done handling it.
	 * 参数[输入] packet Message to deallocate.
	 */
	void DeallocatePacket( Packet *packet );

	/*
	 * 返回允许的总连接数
	 * 返回值: Total number of connections allowed.
	 */
	 ...
	 * 参数[输入] target Target system to cancel.
	 */
	 ...
	 * Values range from 0 to the maximum number of players allowed - 1.
	...
	/*
	 * Given index into remoteSystemList, will return a SystemAddress.
	 ...
	 * 返回值: The SystemAddress structure corresponding to index in remoteSystemList.
	 */
	 ...
	 * 参数[输出] addresses All system addresses. Size of the list is the number of connections. Size of the addresses list will match the size of the guids list.
	 ...
	 * 参数[输入] IP Dotted IP address. You can use * for a wildcard address, such as 128.0.0. * will ban all IP addresses starting with 128.0.0.
	 * 参数[输入] milliseconds Gives time in milli seconds for a temporary ban of the IP address.  Use 0 for a permanent ban.
	 */
	 ...
	 * The remote system, if it is Initialized, will respond with ID_PONG followed by sizeof(RakNet::TimeMS) containing the system time the ping was sent. Default is 4 bytes - See __GET_TIME_64BIT in RakNetTypes.h
	 ...
	 * 参数[输入] data Block of data to send, or 0 for 无
	 * 参数[输入] length Length of the data in bytes, or 0 for 无
	 * 注意: length should be under 400 bytes, as a security measure against flood attacks
	 * 另见 Ping.cpp
	 */
	void SetOfflinePingResponse( const char *data, const unsigned int length );

	/*
	 * 返回指向传递给 SetOfflinePingResponse 的数据副本的指针。
	...
	/*
	 * 返回 unique address identifier that represents you or another system on the the network
	 * 注意: Not supported by the XBOX
	 * 参数[输入] systemAddress Use UNASSIGNED_SYSTEM_ADDRESS to get your behind-LAN address. Use a connected system to get their behind-LAN address. This does not return the port.
	 ...
	 * 返回值: Identifier of your system internally, which may not be how other systems see if you if you are behind a NAT or proxy.
	 */
	...
	/*
	 * Returns the unique address identifier that represents the target on the the network and is based on the target's 外部 IP / port.
	 * 参数[输入] target The SystemAddress of the remote system. Usually the same for all systems, unless you have two or more network cards.
	 */
	...
	/*
	 * Given a connected system address, this method gives the unique GUID representing that instance of RakPeer.
	 ...
	 * 参数[输入] input The system address of the target system we are connected to.
	 */
	const RakNetGUID& GetGuidFromSystemAddress( const SystemAddress input ) const;

	/*
	 * Gives the system address of a connected system, given its GUID.
	 ...
	 * If input is UNASSIGNED_RAKNET_GUID, UNASSIGNED_SYSTEM_ADDRESS is returned.
	 * 参数[输入] input The RakNetGUID of the target system.
	 */
	 ...
	 * 参数[输入] target SystemAddress structure of the target system. Pass UNASSIGNED_SYSTEM_ADDRESS for all systems.
	 */
	void SetTimeoutTime( RakNet::TimeMS timeMS, const SystemAddress target );
	
	/*
	 * 返回 Timeout time for the given system
	 * 参数[输入] target Target system to get the TimeoutTime for. Pass UNASSIGNED_SYSTEM_ADDRESS to get the default value.
	 * 返回值: Timeout time for a given system.
	 */
	RakNet::TimeMS GetTimeoutTime( const SystemAddress target );

	/*
	 * 返回当前 MTU 大小
	 * 参数[输入] target Which system to get MTU for.  UNASSIGNED_SYSTEM_ADDRESS to get the default
	 * 返回值: The current MTU size of the target system.
	 */
	int GetMTUSize( const SystemAddress target ) const;

	/*
	 * 返回此系统内部拥有的 IP 地址数量。
	 * 获取 actual addresses from GetLocalIP()
	 */
	unsigned GetNumberOfAddresses( void );

	/*
	 * 返回 ipList 数组中索引 0 到 GetNumberOfAddresses-1 的 IP 地址。
	...
	/*
	 * Is this a local IP?
	 * 检查 this ip is in the ipList array
	 ...
	 * This will send our 外部 IP outside the LAN along with some user data to the remote system.
	 ...
	 * 参数[输入] interval How many messages to use as an interval before a download progress notification is returned.
	 */
	void SetSplitMessageProgressInterval(int interval);

	/*
	 * 返回传递给 SetSplitMessageProgressInterval() 的值。
	 * 返回值: Number of messages to be recieved before a download progress notification is returned. Default to 0.
	 */
	 ...
	 * 参数[输入] host The address of the remote host in dotted notation.
	 * 参数[输入] remotePort The port number to send to.
	 * 参数[输入] ttl Max hops of datagram, set to 3
	 * 参数[输入] connectionSocketIndex userConnectionSocketIndex.
	 ...
	 * 参数[输入] packet The pointer to the packet you want to push back.
	 * 参数[输入] pushAtHead True to push the packet at the start of the queue so that the next receive call returns it.  False to push it at the end of the queue.
	 * 注意: Setting pushAtHead to false end makes the packets out of order.
	 */
	...
	/*
	 * 返回一个数据包供你写入（如果你因某种原因需要创建一个 Packet）。
	 ...
	 * 返回值: A smart pointer object containing the socket information about the target. Be sure to check IsNull() which is returned if the update thread is unresponsive, shutting down, or if this system is not connected.
	 */
	 ...
	 * 注意: This sends a query to the thread and blocks on the return value for up to one second. In practice it should only take a millisecond or so.
	 * 参数[输出] sockets List of RakNetSocket structures in use.
	 */
	 ...
	 * 参数[输入] rns If you supply this structure,the network statistics will be written to it. Otherwise the method uses a static struct to write the data, which is 非线程安全.
	 * 返回值: 0 if the specified system can't be found. Otherwise a pointer to the struct containing the specified system's network statistics.
	...
	/*
	 * 内部使用
	 * Holds the clock differences between systems, along with the ping
	 */
	...
	/*
	 * 内部使用
	 * All the information representing a connected system
	 */
...
protected:
	...
	/*
	 * 获取 reliability layer associated with a systemAddress
	 * 参数[输入] systemAddress The player identifier
	 * 返回值: 0 if 无
	 */
	...
	/*
	 * 获取 a free remote system from the list and assign our systemAddress to it
	 * 注意: Should only be called from the update thread - not the user thread.
	 * 参数[输入] systemAddress	systemAddress to be assigned
	 * 参数[输入] connectionMode	connection mode of the RemoteSystem.
	 * 参数[输入] rakNetSocket
	 * 参数[输入] thisIPConnectedRecently	Is this IP connected recently? set to False;
	 * 参数[输入] bindingAddress	Address to be binded with the remote system
	 * 参数[输入] incomingMTU	MTU for the remote system
	 */
	 ...
	 * 参数[输入] data	Data in the incoming packet.
	 * 参数[输入] systemAddress Sender of the incoming packet.
	 */
	void ShiftIncomingTimestamp( unsigned char *data, const SystemAddress &systemAddress ) const;
	/*
	 * 获取 most accurate clock differential for a certain player
	 * 参数[输入] systemAddress The player with whose clock the time difference is calculated.
	 * 返回值: The clock differential for a certain player.
	 */
	...
	/* 将this设置为true to terminate the Peer thread execution */
	volatile bool endThreads;
	/* 如果the peer thread is active则为 true */
	...
	/* 05/02/06 改用 maximumNumberOfPeers */
	 ...
	 * 线程安全 because RemoteSystemStruct is preallocated, and the list is only added to, not removed from
	 */
...
/* unsigned int FirstFreeRemoteSystemLookupIndex(const SystemAddress &sa) const; */
	...
	/* RunUpdateCycle is 非线程安全 but we don't need to mutex calls. Just skip calls if it is running already */
