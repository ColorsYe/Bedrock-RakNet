...
/*
 * 内部使用
 * As the name says, a simple multithreaded TCP server.  Used by TelnetTransport
 */
...
public:
	 ...
	 * 参数[输入] port Which port to listen on.
	 * 参数[输入] maxIncomingConnections Max incoming connections we will accept
	 * 参数[输入] maxConnections Max total connections, which should be >= maxIncomingConnections
	...
	/* 获取 how many bytes are waiting to be sent. If too many, you may want to skip sending */
...
protected:
	...
	/* Use this 线程安全 queue to add to remoteClients */
