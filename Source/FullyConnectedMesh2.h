...
/*
 * 全连接网状网络插件，第 2 版
 * This will connect RakPeer to all connecting peers, and all peers the connecting peer knows about.<BR>
 ...
 * 前提条件: You must also install the ConnectionGraph2 plugin in order to use SetConnectOnNewRemoteConnection()
...
public:
	 ...
	 * If attemptConnection is false, you can manually connect to all systems listed in ID_REMOTE_NEW_INCOMING_CONNECTION with ConnectToRemoteNewIncomingConnections()
	 * 注意: This will not work on any console. It will also not work if NAT punchthrough is needed. Generally, this should be false and you should connect manually. It is here for legacy reasons.
	 * 参数[输入] attemptConnection If true, we try to connect to any systems we are notified about with ID_REMOTE_NEW_INCOMING_CONNECTION, which comes from the ConnectionGraph2 plugin. Defaults to true.
	 * 参数[输入] pw The password to use to connect with. Only used if attemptConnection is true
	 */
	 ...
	 * Will return UNASSIGNED_RAKNET_GUID if we are not connected to anyone, or if we are connected and are calculating the host
	 ...
	 * 另见 ConnectionGraph2::GetLowestAveragePingSystem() . If you need one system in the peer to peer group to relay data, have the host call this function after host migration, and use that system
	 * 返回值: System address of whichever system is host.
	 */
	RakNetGUID GetConnectedHost(void) const;
	SystemAddress GetConnectedHostAddr(void) const;

	/* 返回值: System address of whichever system is host. Always returns something, even though it may be our own system. */
	RakNetGUID GetHostSystem(void) const;

	/* 返回值: If our system is host */
	bool IsHostSystem(void) const;

	/*
	 * 获取 list of connected systems, from oldest connected to newest
	 * This is also the order that the hosts will be chosen in
	 */
	void GetHostOrder(DataStructures::List<RakNetGUID> &hostList);

	/*
	 * 参数[输入] includeCalculating If true, and we are currently calculating a new host, return the new host if the calculation is nearly complete
	 * 返回值: If our system is host
	 */
	 ...
	 * Each remote system that you want to check should be added as a participant, either through SetAutoparticipateConnections() or by calling this function
	 * 参数[输入] participant The new participant
	 * 参数[输入] userContext Static data to be passed around with each participant, which can be queried with GetParticipantData().
	 * 另见 StartVerifiedJoin()
	 */
	...
	/*
	 * 返回参与者是否在参与者列表中
	 * 参数[输入] RakNetGUID of the participant to query
	 * 返回值: True if in the list
	 */
	 ...
	 * 参数[输入] RakNetGUID of the participant to query
	 * 参数[输出] userContext Pointer to BitStream to be written to
	 * 返回值: True if data was written
	 */
	/* bool GetParticipantContext(RakNetGUID participantGuid, BitStream *userContext); */

	/*
	 * 将data for other systems设置为read with GetParticipantContext
	 * 参数[输入] userContext Pointer to BitStream to be read from
	 */
	 ...
	 * 参数[输入] packet The packet containing ID_REMOTE_NEW_INCOMING_CONNECTION
	 * 参数[输入] connectionPassword Password passed to RakPeerInterface::Connect()
	 * 参数[输入] connectionPasswordLength Password length passed to RakPeerInterface::Connect()
	 */
	 ...
	 * 2. The client, on ID_FCM2_VERIFIED_JOIN_START, can execute NatPunchthroughClient::OpenNAT() (可选), followed by RakPeerInterface::Connect() if punchthrough success, for each system returned from GetVerifiedJoinRequiredProcessingList()
	 ...
	 * 参数[输入] client The system to send ID_FCM2_VERIFIED_JOIN_START to
	 */
	 ...
	 * 
	 * 参数[输入] packet The system that sent ID_FCM2_VERIFIED_JOIN_CAPABLE. Based on \accept, ID_FCM2_VERIFIED_JOIN_ACCEPTED or ID_FCM2_VERIFIED_JOIN_REJECTED will be sent in reply
	 * 参数[输入] accept True to accept, and thereby automatically call AddParticipant() on all systems on the mesh. False to reject, and call CloseConnection() to all mesh systems on the target
	 * 参数[输入] additionalData Any additional data you want to add to the ID_FCM2_VERIFIED_JOIN_ACCEPTED or ID_FCM2_VERIFIED_JOIN_REJECTED messages
	 * 另见 WriteVJCUserData()
	 */
	 ...
	 * 
	 * 参数[输入] host Which system sent ID_FCM2_VERIFIED_JOIN_START
	 * 参数[输出] addresses SystemAddress values of systems to connect to. List has the same number and order as guids
	 * 参数[输出] guids RakNetGUID values of systems to connect to. List has the same number and order as guids
	 * 参数[输出] userData What was written with WriteVJSUserData
	 */
	 ...
	 * 
	 * 参数[输入] packet Packet containing the ID_FCM2_VERIFIED_JOIN_ACCEPTED message
	 * 参数[输出] thisSystemAccepted If true, it was this instance of RakPeerInterface that was accepted. If false, this is notification for another system
	 * 参数[输出] systemsAccepted Which system(s) were added with AddParticipant(). If thisSystemAccepted is false, this list will only have length 1
	 * 参数[输出] additionalData additionalData parameter passed to RespondOnVerifiedJoinCapable()
	 */
	 ...
	 * 
	 * 参数[输入] packet Packet containing the ID_FCM2_VERIFIED_JOIN_REJECTED message
	 * 参数[输出] additionalData additionalData parameter passed to RespondOnVerifiedJoinCapable().
	 */
	 ...
	 * 参数[输出] bsOut Write your data here, if any. Has to match what is read by ReadVJSUserData
	 * 参数[输入] userGuid The RakNetGuid of the user you are writing for
	 * 参数[输入] userContext The data set with SetMyContext() for that system. May be empty. To properly write userContext, you will need to first write userContext->GetNumberOfBitsUsed(), followed by bsOut->Write(userContext);
	 */
...
protected:
	...
	/* 用于track how long RakNet has been running. This is so we know who has been running longest */
