...
/*
 * 
 * \b [内部使用] Datagram reliable, ordered, unordered and sequenced sends.  Flow control.  Message splitting, reassembly, and coalescence.
 *
 */
...
public:
	...
	/*
	 * 将time, in MS,设置为use before considering ourselves disconnected after not being able to deliver a reliable packet
	 * 默认时间为 Release 模式下 10,000 毫秒（10 秒），Debug 模式下 30,000 毫秒（30 秒）。
	 * 参数[输入] time Time, in MS
	 */
	void SetTimeoutTime( RakNet::TimeMS time );

	/*
	 * 返回 value passed to SetTimeoutTime. or the default if it was never called
	 * 参数[输出] the value passed to SetTimeoutTime
	 */
	 ...
	 * 参数[输入] buffer The socket data
	 * 参数[输入] length The length of the socket data
	 * 参数[输入] systemAddress The player that this data is from
	 * 参数[输入] messageHandlerList A list of registered plugins
	 * 参数[输入] MTUSize maximum datagram size
	 * \retval true Success
	 ...
	 * 参数[输出] data The message
	 * 返回值: Returns number of BITS put into the buffer
	 */
	BitSize_t Receive( unsigned char**data );

	/*
	 * Puts data on the send queue
	 * 参数[输入] data The data to send
	 * 参数[输入] numberOfBitsToSend The length of data in bits
	 * 参数[输入] priority The priority level for the send
	 * 参数[输入] reliability The reliability type for the send
	 * 参数[输入] orderingChannel 0 to 31.  Specifies what channel to use, for relational ordering and sequencing of packets.
	 * 参数[输入] makeDataCopy If true data will be copied.  Otherwise, only a pointer will be stored.
	 * 参数[输入] MTUSize maximum datagram size
	 * 参数[输入] currentTime Current time, as per RakNet::GetTimeMS()
	 * 参数[输入] receipt This number will be returned back with ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS and is only returned with the reliability types that contain RECEIPT in the name
	 * 返回值: True or false for success or failure.
	 */
	...
	/*
	 * Call once per game cycle.  Handles 内部使用 lists and actually does the send.
	 * 参数[输入] s the communication  end point
	 * 参数[输入] systemAddress The Unique Player Identifier who shouldhave sent some packets
	 * 参数[输入] MTUSize maximum datagram size
	 * 参数[输入] time current system time
	 * 参数[输入] maxBitsPerSecond if non-zero, enforces that outgoing bandwidth does not exceed this amount
	 * 参数[输入] messageHandlerList A list of registered plugins
	 */
	 ...
	 * 返回值: true means the connection has been lost.  Otherwise not.
	 */
	...
	/*
	 * 获取 Statistics
	 * 返回值: A pointer to a static struct, filled out with current statistical information.
	 */
	...
	/* 设置 outgoing lag and packet loss properties */
...
private:
	 ...
	 * 参数[输入] s The socket used for sending data
	 * 参数[输入] systemAddress The address and port to send to
	 * 参数[输入] bitStream The data to send.
	 */
	 ...
	 * 返回值: Returns number of bits used
	 */
	...
	/* Parse a bitstream and create an 内部使用 packet to represent this data */
...
/* void DeleteSequencedPacketsInList( unsigned char orderingChannel, DataStructures::Queue<InternalPacket*>&theList ); */
	...
	/* 如果newPacketOrderingIndex is older than the waitingForPacketOrderingIndex则返回 true */
	...
	/*
	 * Creates a copy of the specified 内部使用 packet with data copied from the original starting at dataByteOffset for dataByteLength bytes.
	...
	/* Update all memory which is 非线程安全 */
...
/* unsigned int numPacketsOnResendBuffer; */
	...
	/* 设置 to the current time when the resend queue is no longer empty */
	/* 设置 to zero when it becomes empty */
	/* 设置 to the current time if it is not zero, and we get incoming data */
...
/* CCTimeType lastTimeBetweenPacketsIncrease,lastTimeBetweenPacketsDecrease; */
	...
	/* 将data pointer设置为externallyAllocatedPtr, do not allocate */
