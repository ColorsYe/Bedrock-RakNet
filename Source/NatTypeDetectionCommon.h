...
namespace RakNet
{
	...
	/* All possible types of NATs (except NAT_TYPE_COUNT, which is an 内部使用 value) */
	enum NATTypeDetectionResult
	{
		...
		/* 与 address-restricted cone NAT, but we had to send to both the correct remote IP address and correct remote port. The same source address and port to a different destination uses the same mapping 相同 */
		NAT_TYPE_PORT_RESTRICTED,
		/* A different port is chosen for every remote destination. The same source address and port to a different destination uses a different mapping. Since the port will be different, the first 外部 punchthrough attempt will fail. For this to work it requires port-prediction (MAX_PREDICTIVE_PORT_RANGE>1) and that the router chooses ports sequentially. */
		NAT_TYPE_SYMMETRIC,
	...
	/* 返回值: Can one system with NATTypeDetectionResult type1 connect to type2 */
	...
	/*
	 * 返回表示该枚举的友好字符串
	 * 无 and relaxed can connect to anything
