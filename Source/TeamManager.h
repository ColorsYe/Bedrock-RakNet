 ...
 *  TeamManager.h
 * 自动化团队的网络通信和列表管理
 * TeamManager provides support for teams. A team is a list of team members.
...
struct TeamSelection
{
	 ...
	 * ID_TEAM_BALANCER_TEAM_ASSIGNED, ID_TEAM_BALANCER_REQUESTED_TEAM_FULL, or ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED will be returned to all systems.
	 * 参数[输入] specificTeamToJoin Which team to attempt to join.
	 */
	 ...
	 * 参数[输入] noTeamSubcategory Even when not on a team, you can internally identify a subcategory of not being on a team, such as AI or spectator.
	 */
 ...
 * Contains data and operations on data to manage which team your game's team members are on.
...
public:
	 ...
	 * Function will return false on invalid operations, such as joining a team you are already on.
	 ...
	 * 注意: Joining a specific team with this function may result in being on more than one team at once, even if you call the function while locally only on one team. If your game depends on only being on one team at a team, use RequestTeamSwitch() instead with the parameter teamToLeave set to 0
	 * 参数[输入] TeamSelection::AnyAvailable(), TeamSelection::SpecificTeam(), or TeamSelection::NoTeam()
	 * 返回值: false On invalid or unnecessary operation. Otherwise returns true
	 */
	 ...
	 * 参数[输入] teamToJoin Which team to join
	 * 参数[输入] teamToLeave If 0, means leave all current teams. Otherwise, leave the specified team.
	 * 返回值: false On invalid or unnecessary operation. Otherwise returns true
	 */
	bool RequestTeamSwitch(TM_Team *teamToJoin, TM_Team *teamToLeave);
	
	/*
	 * 返回 first requested team in the list of requested teams, if you have a requested team at all
	 * 返回值: TeamSelection::SpecificTeam(), TeamSelection::NoTeam(), or TeamSelection::AnyAvailable()
	 */
	 ...
	 * 参数[输出] All pending requested teams
	 */
	 ...
	 * 参数[输入] The team we are checking
	 * 返回值: Did we request to join this specific team?
	 */
	bool HasRequestedTeam(TM_Team *team) const;

	/*
	 * 返回 index of team in the requested teams list
	 * 参数[输入] The team we are checking
	 * 返回值: -1 if we did not requested to join this team. Otherwise the index.
	 */
	unsigned int GetRequestedTeamIndex(TM_Team *team) const;

	/* 返回值: The number of teams that would be returned by a call to GetRequestedSpecificTeams() */
	 ...
	 * 注意: This is not guaranteed to work due to latency. To clarify, If the host switches your team at the same time you call CancelRequestTeam() you may still get ID_TEAM_BALANCER_TEAM_ASSIGNED for the team you tried to cancel.
	 * 参数[输入] specificTeamToCancel Which team to no longer join. Use 0 for all.
	 * 返回值: false On invalid or unnecessary operation. Otherwise returns true
	 */
	...
	/*
	 * Leave a team
	 * Leaves a team that you are on. Always succeeds provided you are on that team
	 * Generates ID_TEAM_BALANCER_TEAM_ASSIGNED on all systems on success.
	 * If you leave the last team you are on, noTeamSubcategory is set as well.
	 * 参数[输入] team Which team to leave
	 * 参数[输入] _noTeamSubcategory If the team member has been removed from all teams, which subcategory of NoTeamId to set them to
	 * 返回值: false On invalid or unnecessary operation. Otherwise returns true
	 */
	...
	/*
	 * Leave all teams
	 * Leaves all teams you are on, and sets noTeamSubcategory
	 * 注意: This is the same as and just calls RequestTeam(TeamSelection::NoTeam(noTeamSubcategory));
	 * 返回值: false On invalid or unnecessary operation. Otherwise returns true
	 */
	bool LeaveAllTeams(NoTeamId noTeamSubcategory);
	
	/* 返回值: Get the first team we are on, or 0 if we are not on a team. */
	TM_Team* GetCurrentTeam(void) const;

	/* 返回值: How many teams we are on */
	unsigned int GetCurrentTeamCount(void) const;

	/* 返回值: Returns one of the teams in the current team list, up to GetCurrentTeamCount() */
	TM_Team* GetCurrentTeamByIndex(unsigned int index);
	
	/* 参数[输出] Get all teams we are on, as a list */
	 ...
	 * 参数[输出] _teams The previous list of teams we were on
	 */
	void GetLastTeams(DataStructures::List<TM_Team*> &_teams) const;

	/*
	 * 参数[输入] The team we are checking
	 * 返回值: Are we on this team?
	 */
	bool IsOnTeam(TM_Team *team) const;
	
	/* 返回值: The teamMemberID parameter passed to TM_World::ReferenceTeamMember() */
	NetworkID GetNetworkID(void) const;
	
	/* 返回值: The TM_World instance that was used when calling TM_World::ReferenceTeamMember() */
	TM_World* GetTM_World(void) const;
	
	/*
	 * Serializes the current state of this object
	 * To replicate a TM_TeamMember on another system, first instantiate the object using your own code, or a system such as ReplicaManager3.
	 ...
	 * 注意: You must instantiate and deserialize all TM_Team instances that the team member refers to before calling DesrializeConstruction(). ReplicaManager3::PostSerializeConstruction() and ReplicaManager3::PostDeserializeConstruction() will ensure this.
	 * 参数[输出] constructionBitstream This object serialized to a BitStream
	 */
	void SerializeConstruction(BitStream *constructionBitstream);
	
	/*
	 * Deserializes the current state of this object
	 * 参见 SerializeConstruction for more details()
	 * 注意: DeserializeConstruction also calls ReferenceTeamMember on the passed teamManager instance, there is no need to do so yourself
	 * 参数[输入] teamManager TeamManager instance
	 * 参数[输入] constructionBitstream This object serialized to a BitStream
	 */
	bool DeserializeConstruction(TeamManager *teamManager, BitStream *constructionBitstream);

	/* 参数[输入] o Stores a void* for your own use. If using composition, this is useful to store a pointer to the containing object. */
	void SetOwner(void *o);

	/* 返回值: Whatever was passed to SetOwner() */
	void *GetOwner(void) const;

	/* 返回值: If not on a team, returns the current NoTeamId value */
 ...
 * 包含 lists of TM_TeamMember instances
...
public:
	...
	/*
	 * 设置 maximum number of members that can join this team
	 * Defaults to 65535
	 ...
	 * 参数[输入] _teamMemberLimit The new limit
	 * 参数[输入] noTeamSubcategory Which noTeamSubcategory to assign to members that now have no team.
	 * 返回值: false On invalid or unnecessary operation. Otherwise returns true
	 */
	bool SetMemberLimit(TeamMemberLimit _teamMemberLimit, NoTeamId noTeamSubcategory);
	
	/* 返回值: If team balancing is on, the most members that can be on this team that would not either unbalance it or exceed the value passed to SetMemberLimit(). If team balancing is off, the same as GetMemberLimitSetting() */
	TeamMemberLimit GetMemberLimit(void) const;

	/* 返回值: What was passed to SetMemberLimit() or the default */
	TeamMemberLimit GetMemberLimitSetting(void) const;
	
	/*
	 * Who can join this team under what conditions, while the team is not full
	 ...
	 * 参数[输入] _joinPermissions Bitwise combination of ALLOW_JOIN_ANY_AVAILABLE_TEAM, ALLOW_JOIN_SPECIFIC_TEAM, ALLOW_JOIN_REBALANCING
	 * 返回值: false On invalid or unnecessary operation. Otherwise returns true
	 */
	bool SetJoinPermissions(JoinPermissions _joinPermissions);
	
	/* 返回值: Whatever was passed to SetJoinPermissions(), or the default. */
	 ...
	 * 参数[输入] teamMember Which team member to remove
	 * 参数[输入] noTeamSubcategory If the team member has been removed from all teams, which subcategory of NoTeamId to set them to
	 */
	void LeaveTeam(TM_TeamMember* teamMember, NoTeamId noTeamSubcategory);
	
	/* 返回值: What was passed as the applyBalancing parameter TM_World::ReferenceTeam() when this team was added. */
	bool GetBalancingApplies(void) const;
	
	/* 参数[输出] All team members of this team */
	void GetTeamMembers(DataStructures::List<TM_TeamMember*> &_teamMembers) const;

	/* 返回值: The number of team members on this team */
	unsigned int GetTeamMembersCount(void) const;

	/*
	 * 返回值: A team member on this team. Members are stored in the order they are added
	 * 参数[输入] index A value between 0 and GetTeamMembersCount()
	 */
	TM_TeamMember *GetTeamMemberByIndex(unsigned int index) const;
	
	/* 返回值: The teamID parameter passed to TM_World::ReferenceTeam() */
	NetworkID GetNetworkID(void) const;
	...
	/*
	 * 被the host to serialize the initial state of this object to a new system使用
	 * On the host, when sending existing objects to a new system, call SerializeConstruction() on each of those objects to serialize creation state.
	 * Creating the actual Team and TeamMember objects should be handled by your game code, or a system such as ReplicaManager3
	 */
	void SerializeConstruction(BitStream *constructionBitstream);
	
	/*
	 * 被non-host systems to read the bitStream written by SerializeConstruction()使用
	 * On non-host systems, after creating existing objects, call DeserializeConstruction() to read and setup that object
	 * Creating the actual Team and TeamMember objects should be handled by your game code, or a system such as ReplicaManager3
	 */
	bool DeserializeConstruction(TeamManager *teamManager, BitStream *constructionBitstream);
 ...
 * Each TM_World instance is independent of other TM_World world instances. This enables you to host multiple games on a single computer.
...
public:
	...
	/* 返回值: Returns the plugin that created this TM_World instance */
	 ...
	 * 参数[输入] rakNetGUID GUID of the system you are removing. See Packet::rakNetGUID or RakPeerInterface::GetGUIDFromSystemAddress()
	 */
	 ...
	 * Your game should contain instances of TM_Team, for example by using composition with your game's Team or PlayerList class
	 * Tell TeamManager about these instances using ReferenceTeam().
	 * 注意: The destrutor of TM_Team calls DereferenceTeam() automatically.
	 * 参数[输入] team The instance you are registering
	 * 参数[输入] networkId Identifies this instance. This value is independent of values used by NetworkIDManager. You can use the same value as the object that contains this instance.
	 * 参数[输入] applyBalancing Whether or not to include this team for balancing when calling SetBalanceTeams().
	 */
	void ReferenceTeam(TM_Team *team, NetworkID networkId, bool applyBalancing);

	/*
	 * Unregisters the associated TM_Team object with this system.
	 * Call when a TM_Team instance is no longer needed
	 * 参数[输入] team Which team instance to unregister
	 * 参数[输入] noTeamSubcategory All players on this team are kicked off. If these players then have no team, they are set to this no team category.
	 */
	void DereferenceTeam(TM_Team *team, NoTeamId noTeamSubcategory);

	/* 返回值: Number of teams uniquely added with ReferenceTeam() */
	unsigned int GetTeamCount(void) const;

	/*
	 * 参数[输入] index A value between 0 and GetTeamCount()
	 * 返回值: Returns whatever was passed to team in the function ReferenceTeam() in the order it was called.
	 */
	TM_Team *GetTeamByIndex(unsigned int index) const;

	/*
	 * 参数[输入] teamId Value passed to ReferenceTeam()
	 * 返回值: Returns whatever was passed to team in the function ReferenceTeam() with this NetworkID.
	 */
	TM_Team *GetTeamByNetworkID(NetworkID teamId);

	/*
	 * Inverse of GetTeamByIndex()
	 * 参数[输入] team Which taem
	 * 返回值: The index of the specified team, or -1 if not found
	 */
	unsigned int GetTeamIndex(const TM_Team *team) const;

	/*
	 * Register a TM_TeamMember object with this system.
	 * Your game should contain instances of TM_TeamMember, for example by using composition with your game's User or Player classes
	 * Tell TeamManager about these instances using ReferenceTeamMember().
	 * 注意: The destrutor of TM_TeamMember calls DereferenceTeamMember() automatically.
	 * 参数[输入] teamMember The instance you are registering
	 * 参数[输入] networkId Identifies this instance. This value is independent of values used by NetworkIDManager. You can use the same value as the object that contains this instance
	 */
	void ReferenceTeamMember(TM_TeamMember *teamMember, NetworkID networkId);

	/*
	 * Unregisters the associated TM_TeamMember object with this system.
	 * Call when a TM_TeamMember instance is no longer needed
	 * 注意: This is called by the 析构函数 of TM_TeamMember automatically, so you do not normally need to call this function
	 */
	void DereferenceTeamMember(TM_TeamMember *teamMember);

	/* 返回值: Number of team members uniquely added with ReferenceTeamMember() */
	unsigned int GetTeamMemberCount(void) const;

	/*
	 * 参数[输入] index A value between 0 and GetTeamMemberCount()
	 * 返回值: Returns whatever was passed to team in the function ReferenceTeamMember() in the order it was called.
	 */
	TM_TeamMember *GetTeamMemberByIndex(unsigned int index) const;

	/*
	 * 参数[输入] index A value between 0 and GetTeamMemberCount()
	 * 返回值: Returns whatever was passed to teamMemberID in the function ReferenceTeamMember() in the order it was called.
	 */
	NetworkID GetTeamMemberIDByIndex(unsigned int index) const;

	/*
	 * 参数[输入] teamId Value passed to ReferenceTeamMember()
	 * 返回值: Returns Returns whatever was passed to team in the function ReferenceTeamMember() with this NetworkID
	 */
	TM_TeamMember *GetTeamMemberByNetworkID(NetworkID teamMemberId);

	/*
	 * Inverse of GetTeamMemberByIndex()
	 * 参数[输入] team Which team member
	 * 返回值: The index of the specified team member, or -1 if not found
	 */
	unsigned int GetTeamMemberIndex(const TM_TeamMember *teamMember) const;

	/*
	 * Force or stop forcing teams to be balanced.
	 * For each team added with ReferenceTeam() and applyBalancing set to true, players on unbalanced teams will be redistributed
	 ...
	 * 参数[输入] balanceTeams Whether to activate or deactivate team balancing.
	 * 参数[输入] noTeamSubcategory If a player is kicked off a team and is no longer on any team, his or her noTeamSubcategory is set to this value
	 */
	bool SetBalanceTeams(bool balanceTeams, NoTeamId noTeamSubcategory);

	/* 返回值: balanceTeams parameter of SetBalanceTeams(), or the default */
	bool GetBalanceTeams(void) const;

	/*
	 * 设置 host that will perform balancing calculations and send notifications
	 * Operations that can cause conflicts due to latency, such as joining teams, are operated on by the host. The result is sent to all systems added with AddParticipant()
	 ...
	 * 注意: If using FullyConnectedMesh2, SetHost() is called automatically when ID_FCM2_NEW_HOST is returned.
	 * 参数[输入] _hostGuid The host, which is the system that will serialize and resolve team disputes and calculate team balancing.
	 */
	void SetHost(RakNetGUID _hostGuid);

	/* 返回值: Returns the current host, or UNASSIGNED_RAKNET_GUID if unknown */
	RakNetGUID GetHost(void) const;

	/* 返回值: The worldId passed to TeamManagr::AddWorld() */
...
protected:
	...
	void OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, bool isIncoming);
 ...
 * 注意: This replaces TeamBalancer. You cannot use TeamBalancer and TeamManager at the same time.
 * \ingroup TEAM_MANAGER_GROUP
 */
...
public:
	...
	/*
	 * Allocate a world to hold a list of teams and players for that team.
	 * Use the returned TM_World object for actual team functionality.
	 * 注意: The world is tracked by TeamManager and deallocated by calling Clear()
	 * 参数[输入] worldId Arbitrary user-defined id of the world to create. Each world instance must have a unique id.
	 */
	TM_World* AddWorld(WorldId worldId);

	/*
	 * Deallocate a world created with AddWorld()
	 * 参数[输入] worldId The world to deallocate
	 */
	void RemoveWorld(WorldId worldId);

	/* 返回值: Returns the number of worlds created with AddWorld() */
	unsigned int GetWorldCount(void) const;

	/*
	 * 参数[输入] index A value beteween 0 and GetWorldCount()-1 inclusive.
	 * 返回值: Returns a world created with AddWorld()
	 */
	TM_World* GetWorldAtIndex(unsigned int index) const;

	/*
	 * 参数[输入] worldId worldId value passed to AddWorld()
	 * 返回值: Returns a world created with AddWorld(), or 0 if no such worldId
	 */
	TM_World* GetWorldWithId(WorldId worldId) const;
	
	/*
	 * When auto managing connections, call TM_World::AddParticipant() on all worlds for all new connections automatically
	 * 默认为 true
	 * 注意: You probably want this set to false if using multiple worlds
	 * 参数[输入] autoAdd Automatically call TM_World::AddParticipant() all worlds each new connection. Defaults to true.
	 */
	void SetAutoManageConnections(bool autoAdd);
	 ...
	 * 注意: If TM_PEER_TO_PEER, this plugin will listen for ID_FCM2_NEW_HOST and call SetHost() on all worlds automatically
	 * 注意: Defaults to TM_PEER_TO_PEER
	 * 参数[输入] _topology Topology to use
	 */
	 ...
	 * 参数[输入] A packet where packet->data[0]==ID_TEAM_BALANCER_REQUESTED_TEAM_FULL
	 * 返回值: true on success, false on read error
	 */
	 ...
	 * 参数[输入] A packet where packet->data[0]==ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED
	 * 返回值: true on success, false on read error
	 */
	 ...
	 * 注意: You can get the current and prior team list from the teamMember itself
	 * 参数[输入] A packet where packet->data[0]==ID_TEAM_BALANCER_TEAM_ASSIGNED
	 * 参数[输出] world 设置 to the world this teamMember is on. 0 on bad lookup
	 * 参数[输出] teamMember 设置 to the teamMember affected. 0 on bad lookup
	 */
	void DecodeTeamAssigned(Packet *packet, TM_World **world, TM_TeamMember **teamMember);

	/* Reads out the world and teamMember from ID_TEAM_BALANCER_TEAM_REQUESTED_CANCELLED */
	/*
	 * 注意: You can get the requested team list from the teamMember itself
	 * 参数[输入] A packet where packet->data[0]==ID_TEAM_BALANCER_TEAM_REQUESTED_CANCELLED
	 * 参数[输出] world 设置 to the world this teamMember is on. 0 on bad lookup
	 * 参数[输出] teamMember 设置 to the teamMember affected. 0 on bad lookup
	 * 参数[输出] teamCancelled 设置 to the team that was cancelled. 0 for all teams
	 */
