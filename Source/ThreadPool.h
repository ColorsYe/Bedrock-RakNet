...
struct RAK_DLL_EXPORT ThreadPool
{
	 ...
	 * 参数[输入] numThreads The number of threads to start
	 * 参数[输入] stackSize 0 for default (except on consoles).
	 * 参数[输入] _perThreadInit User callback to return data stored per thread.  Pass 0 if not needed.
	 * 参数[输入] _perThreadDeinit User callback to destroy data stored per thread, created by _perThreadInit.  Pass 0 if not needed.
	 ...
	 * The best way to deallocate inputData is in userCallback.  If you call EndThreads such that callbacks were not called, you
	 ...
	 * 参数[输入] workerThreadCallback The function to call from the thread
	 * 参数[输入] inputData The parameter to pass to userCallback
	 */
	 ...
	 * 参数[输入] outputData The output to inject
	 */
	void AddOutput(OutputType outputData);

	/*
	 * 如果output from GetOutput is waiting则返回 true
	 * 返回值: true if output is waiting, false otherwise
	 */
	bool HasOutput();

	/*
	 * Inaccurate but fast version of HasOutput.  If this returns true, you should still check HasOutput for the real value.
	 * 返回值: true if output is probably waiting, false otherwise
	 */
	bool HasOutputFast();

	/*
	 * 如果input from GetInput is waiting则返回 true
	 * 返回值: true if input is waiting, false otherwise
	 */
	bool HasInput();

	/*
	 * Inaccurate but fast version of HasInput.  If this returns true, you should still check HasInput for the real value.
	 * 返回值: true if input is probably waiting, false otherwise
	 */
	bool HasInputFast();

	/*
	 * 获取 output of a call to userCallback
	 * HasOutput must return true before you call this function.  Otherwise it will assert.
	 * 返回值: The output of userCallback.  If you have different output signatures, it is up to you to encode the data to indicate this
	 */
	...
	/* 获取 output at a specified index */
